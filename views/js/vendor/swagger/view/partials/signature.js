  ...
  'taoRestApiDocs/vendor/swagger/SwaggerUi'
    ...
    SwaggerUi.partials.signature = (function () {
        ...
        var createPrimitiveXML = function (descriptor) {
            ...
            if (_.keys(primitivesMap).indexOf(type) < 0) {
                return getErrorMessage();
            }
        ...
        function createSchemaXML(name, definition, models, config) {
            ...
            switch (descriptor.type) {
                case 'array':
                    output = createArrayXML(descriptor);
                    break;
                case 'object':
                    output = createObjectXML(descriptor);
                    break;
                case 'loop':
                    output = getInfiniteLoopMessage(descriptor.name, descriptor.config.loopTo);
                    break;
                default:
